...
```{r}
#use anova
Tool_model <- lm(Value ~ A + B + C, data = Tool_data)
anova(Tool_model)
```
...
```{r}
#one of the easiest ways to make a normal probability plot is through the rstandard function

qqnorm(Tool_data$Value)
...
```{r}
anova(lm(Value ~ A + B + C + D, data = Soft_drink_data))
```
